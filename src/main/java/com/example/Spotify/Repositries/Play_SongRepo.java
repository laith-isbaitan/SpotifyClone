...
public interface Play_SongRepo extends CrudRepository<Playlist_song, Long> {
	...
	List<Object[]> findByAllAddsforSong2();
	
	@Query(value = "select songs.title, songs.artist, sum(num_of_times_added) "
			+ "from songs join playlist_song on playlist_song.song_id = songs.id group by songs.id;", nativeQuery = true)
	List<Object[]> findByAllAddsforSong3();
}